...
class GRUCNNDecoder(nn.Module):
    def __init__(self,
        ...
        # ---------------------------------------------------
        # 🔥 CNN FRONT-END (PARTE NOVA)
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=neural_dim, out_channels=256, kernel_size=5, padding=2),
            nn.ReLU(),
            nn.Conv1d(in_channels=256, out_channels=neural_dim, kernel_size=3, padding=1),
            nn.ReLU()
        )
        # ---------------------------------------------------
    ...
    def forward(self, x, day_idx, states=None, return_state=False):
        ...
        # 🔥 CNN FRONT-END (PARTE NOVA)
        # ---------------------------------------------------
        # CNN espera: (B, channels, T)
        x = x.permute(0, 2, 1)            # (B, 512, T)

        x = self.cnn(x)                   # (B, 512, T)  ou (B, C_out, T)

        x = x.permute(0, 2, 1)            # volta para (B, T, features)
        # ---------------------------------------------------
